      ...
      <div className="px-4 mt-4 space-y-4">
        ...
        <Card className="p-4 rounded-xl border-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>显示</span>
              <Switch checked={visibility?.showCarAllowance ?? true} onCheckedChange={(v) => setVisibility.mutate({ showCarAllowance: v })} />
            </div>
          </div>
          {carStatus ? (
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className={`font-medium ${carStatus.directVips >= 12 ? "text-green-600" : "text-foreground"}`}>{carStatus.directVips} / 12 人</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">本月团队业绩</span>
                <span className={`font-medium ${carStatus.performance >= 18000 ? "text-green-600" : "text-foreground"}`}>{formatRM(carStatus.performance)} / RM18,000</span>
              </div>
        ...
        <Card className="p-4 rounded-xl border-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>显示</span>
              <Switch checked={visibility?.showTravelReward ?? true} onCheckedChange={(v) => setVisibility.mutate({ showTravelReward: v })} />
            </div>
          </div>
          {travelStatus && (
